...
| Requirement | Where |
| --- | --- |
| Total file size ≤ 250 MB, ≤ 1500 files | bundle is ~8 MB / ~55 files (`bun run build:cg`) |
| Initial download ≤ 50 MB (≤ 20 MB for the mobile homepage) | the platform stops counting at the first `gameplayStart`, which is the moment the car becomes drivable: index + Drive chunk + the 3.2 MB built map + the selected car (~6 MB). No model is preloaded in the garage. |
| **Relative paths only inside the bundle** | every shipped asset path is bundle-relative (`maps/riverbend.glb`, `maps/riverbend.props.json`, `models/*.glb`, `models/props/*.glb`, `tex/*.png`) and the Draco decoder is resolved with `new URL("draco/gltf/", document.baseURI)`. Owner-imported cars/maps/props are the only absolute URLs (they live on the backend). The build uses `--base=./` so the emitted HTML/CSS/JS references are relative too. |
...
| Assets load from the bundle, nothing is fetched at run time that can be avoided | the furniture of the world is real models (`public/models/props/`, CC0 Kenney kits) drawn as instances over spots the world recorded (`src/game/props.ts`); the shipped map's own prop list is a 28 KB JSON next to it. Owner-imported props are the only remote models, and a model that fails to load leaves its low-poly twin standing. |
...
bun src/tools/check-props.ts               # where the world stands its props
